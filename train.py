...
from kandimate.data.dataset import WebVid10M
...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    tokenizer    = CLIPTokenizer.from_pretrained(pretrained_kandinsky_prior, subfolder='tokenizer')
    text_encoder = CLIPTextModelWithProjection.from_pretrained(
        pretrained_kandinsky_prior, subfolder="text_encoder", torch_dtype=torch.float32,
    )
    vae          = VQModel.from_pretrained(pretrained_kandinsky_decoder, subfolder="movq", torch_dtype=torch.float32)
    if not image_finetune:
        ...
        raise NotImplementedError("Only temporal layers training is availabe.")

    pipe_prior = KandinskyV22PriorPipeline.from_pretrained(pretrained_kandinsky_prior, torch_dtype=torch.float32)
    ...
    # Validation pipeline
    if not image_finetune:
        validation_pipeline = KandimatePipeline(
            unet=unet, movq=vae, tokenizer=tokenizer, text_encoder=text_encoder, scheduler=noise_scheduler,
        ).to('cuda')
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            pixel_values = batch["pixel_values"].to(device=local_rank, dtype=torch.float32)
            ...
            with torch.no_grad():
                ...
                image_embeds, zero_image_embeds = pipe_prior(
                        prompt=batch['text'], negative_prompt=[""] * len(batch['text']), #generator=generator, 
                    ).to_tuple()
                ...
                for idx, prompt in enumerate(prompts):
                    if not image_finetune:
                        image_emb, zero_image_emb = pipe_prior(
                            prompt=prompt, negative_prompt='', generator=generator, 
                        ).to_tuple()
                        ...
                        sample = validation_pipeline(
                            ...
                            guidance_scale = validation_data.get("guidance_scale", 4),
                        ).videos
