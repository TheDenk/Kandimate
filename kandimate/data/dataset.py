...
class MxkitLowMem(Dataset):
    def __init__(
        ...
        df = pd.read_csv(csv_path)
        df['video_name'] = df['Href'].apply(lambda x: os.path.basename(x))
        df = df.set_index('video_name')
        
        video_names = [os.path.basename(x) for x in self.videos_paths]
        self.df = df[df.index.isin(video_names)]
    ...
    def get_batch(self, idx):
        ...
        image_embeds = self.embeds[video_name]["image_embeds"]
        prompt_embeds = self.embeds[video_name]["prompt_embeds"]
